...
(function(root,factory){
  ...
  const PERFORMANCE_POLICY=Object.freeze({
    rhythmTiming:{evidenceType:'USER_MEASUREMENT',skillRefs:['rhythm'],baseReliability:.78,plannerWeight:.22,recommendable:true,minSamples:4,targetSamples:32},
    polyrhythmTiming:{evidenceType:'USER_MEASUREMENT',skillRefs:['rhythm'],baseReliability:.82,plannerWeight:.24,recommendable:true,minSamples:4,targetSamples:16},
    melodyMemory:{evidenceType:'USER_MEASUREMENT',skillRefs:['melody'],baseReliability:.80,plannerWeight:.24,recommendable:true},
    harmonyFunction:{evidenceType:'USER_MEASUREMENT',skillRefs:['progression','modulation'],baseReliability:.78,plannerWeight:.22,recommendable:true},
    midiPerformance:{evidenceType:'DEVICE_ASSISTED_RESPONSE',skillRefs:['melody','chord'],baseReliability:.62,plannerWeight:0,recommendable:false}
  });
  ...
  function performanceQualification(kind,metrics={},requestedReliability=null){
    const key=String(kind||''),policy=performancePolicy(key);if(!policy)return{valid:false,evidenceType:'LEGACY_DIAGNOSTIC',skillRefs:[],reliability:0,plannerWeight:0,plannerEligible:false,sampleCount:null,targetSamples:null,minSamples:null,reason:'unknown_performance_kind'};
    const base=clamp(requestedReliability??policy.baseReliability,0,policy.baseReliability);let valid=true,sampleCount=null,reason=null;
    ...
    const sufficiency=policy.targetSamples&&sampleCount!=null?Math.sqrt(clamp(sampleCount/policy.targetSamples,0,1)):1,reliability=valid?clamp(base*sufficiency,0,policy.baseReliability):0,plannerEligible=Boolean(valid&&policy.plannerWeight>0);
    return{valid,evidenceType:valid?policy.evidenceType:'INVALID_MEASUREMENT',skillRefs:[...policy.skillRefs],reliability,plannerWeight:plannerEligible?policy.plannerWeight:0,plannerEligible,sampleCount,targetSamples:policy.targetSamples||null,minSamples:policy.minSamples||null,reason};
  }
  function sanitizePerformanceRow(x){if(!x||typeof x!=='object')return null;const kind=String(x.kind||'').slice(0,32),policy=performancePolicy(kind),diagnostic=!policy,metrics=x.metrics&&typeof x.metrics==='object'?JSON.parse(JSON.stringify(x.metrics)):{},q=policy?performanceQualification(kind,metrics,x.reliability):null;return{time:Math.max(0,finite(x.time)),kind,score:clamp(x.score,0,1),metrics,evidenceType:q?.evidenceType||(LEGACY_DIAGNOSTIC_KINDS.has(kind)?'DERIVED_DIAGNOSTIC':'LEGACY_DIAGNOSTIC'),skillRefs:q?[...q.skillRefs]:[],reliability:q?.reliability||0,plannerWeight:q?.plannerWeight||0,plannerEligible:Boolean(q?.plannerEligible),measurementValid:q?.valid===true,sampleCount:q?.sampleCount??null,sampleTarget:q?.targetSamples??null,qualificationReason:q?.reason||null,legacyDiagnostic:diagnostic}}
  ...
  function targetingPerformanceRow(raw){if(!raw||typeof raw!=='object')return null;const kind=String(raw.kind||'').slice(0,32),policy=performancePolicy(kind);if(!policy)return null;const metrics=raw.metrics&&typeof raw.metrics==='object'?raw.metrics:{},q=performanceQualification(kind,metrics,raw.reliability);return{time:Math.max(0,finite(raw.time)),kind,score:clamp(raw.score,0,1),skillRefs:q.skillRefs,reliability:q.reliability,plannerWeight:q.plannerWeight,plannerEligible:q.plannerEligible}}
  function performanceTargetingByFamily(state,timestamp=Date.now()){
    const rows=state.performance||[];if(rows.length===0)return Object.create(null);const latest=Object.create(null);for(const raw of rows){const row=targetingPerformanceRow(raw),policy=performancePolicy(row?.kind);if(!row||!policy||policy.recommendable!==true||!row.plannerEligible||!(row.plannerWeight>0)||!(row.reliability>0))continue;if(row.time>timestamp||timestamp-row.time>14*DAY)continue;if(!latest[row.kind]||row.time>latest[row.kind].time)latest[row.kind]=row}
    const out=Object.create(null);for(const row of Object.values(latest)){const policy=performancePolicy(row.kind),need=clamp((.75-row.score)/.75,0,1),rel=clamp(row.reliability/Math.max(.0001,policy.baseReliability),0,1),weight=clamp(row.plannerWeight/.24,0,1),pressure=clamp(need*rel*weight,0,1);if(!(pressure>0))continue;for(const ref of row.skillRefs||[]){const prior=clamp(out[ref]||0,0,1);out[ref]=1-(1-prior)*(1-pressure)}}return out;
  }
  function scoreUnit(state,unit,{mode='practice',timestamp=Date.now(),preparedFamily=null,preparedCard=undefined,preparedRepeatId=undefined,preparedRepeatFamily=undefined,preparedTargeting=undefined}={}){
    ...
    const baseScore=Math.max(.04,.15+novelty+weakness+uncertainty+due+confusion+fairness+challenge+latency+repeat+moduleWeight+psychometric),targeting=mode==='practice'?(preparedTargeting===undefined?performanceTargetingByFamily(state,timestamp):preparedTargeting):null,targetingPressure=clamp(targeting?.[unit.family]||0,0,1);
  ...
  function selectUnit(state,catalog,{mode='practice',focus='adaptive',timestamp=Date.now(),seed=`${timestamp}:${state.counters?.[mode]||0}`}={}){
    ...
    const random=seeded(seed),targeting=mode==='practice'&&focus==='adaptive'?performanceTargetingByFamily(state,timestamp):null;
    let selected;
    if(random()<.14){
      ...
      const k=list.indexOf(selected),score=k>=0?weights[k]:scoreUnit(state,selected,{mode,timestamp,preparedTargeting:targeting});
      return{unit:selected,score,eligible:list.length,prediction:prediction(state,selected,timestamp)};
    }
    return{unit:selected,score:scoreUnit(state,selected,{mode,timestamp,preparedTargeting:targeting}),eligible:list.length,prediction:prediction(state,selected,timestamp)};
  }
  ...
  function recordPerformance(state,{kind,score=0,metrics={},reliability=null,timestamp=Date.now()}={}){const key=String(kind||''),policy=performancePolicy(key);if(!policy)throw new Error(`Type de preuve performance interdit: ${key||'absent'}`);const cleanMetrics=metrics&&typeof metrics==='object'?JSON.parse(JSON.stringify(metrics)):{},q=performanceQualification(key,cleanMetrics,reliability),row={time:timestamp,kind:key,score:clamp(score,0,1),metrics:cleanMetrics,evidenceType:q.evidenceType,skillRefs:[...q.skillRefs],reliability:q.reliability,plannerWeight:q.plannerWeight,plannerEligible:q.plannerEligible,measurementValid:q.valid,sampleCount:q.sampleCount,sampleTarget:q.targetSamples,qualificationReason:q.reason,legacyDiagnostic:false};state.performance||(state.performance=[]);state.performance.push(row);if(state.performance.length>240)state.performance.splice(0,state.performance.length-240);state.updated=timestamp;return row}
  function performanceEvidenceSignals(state,timestamp=Date.now()){const latest={},rows=[];for(const raw of state.performance||[]){const row=sanitizePerformanceRow(raw),policy=performancePolicy(row?.kind);if(!row||!policy||!row.plannerEligible||!(row.plannerWeight>0))continue;rows.push(row);if(!latest[row.kind]||row.time>latest[row.kind].time)latest[row.kind]=row}let weight=0,deficit=0;for(const row of Object.values(latest)){const w=row.reliability*row.plannerWeight;weight+=w;deficit+=(1-row.score)*w}const recommendable=Object.entries(PERFORMANCE_POLICY).filter(([,p])=>p.recommendable),staleness=recommendable.map(([kind])=>{const row=latest[kind];return row?clamp((timestamp-row.time)/(14*DAY),0,1):1});return{eligibleEvidence:rows.length,latestKinds:Object.keys(latest).sort(),weightedDeficit:weight?deficit/weight:0,measurementNeed:staleness.length?staleness.reduce((a,b)=>a+b,0)/staleness.length:0}}
  function recommendedProbe(state,catalog,timestamp=Date.now()){if(!catalog)return null;const by=Object.fromEntries(summarize(state,catalog,timestamp).families.map(x=>[x.family,x])),last={};for(const raw of state.performance||[]){const row=sanitizePerformanceRow(raw),policy=performancePolicy(row?.kind);if(row&&row.plannerEligible&&policy?.recommendable&&(!last[row.kind]||row.time>last[row.kind].time))last[row.kind]=row}const defs=[{kind:'rhythmTiming',label:'Pulsation et microtiming',ready:()=>((by.rhythm?.practiced||0)>=2),weak:()=>1-(by.rhythm?.mastery||0)},{kind:'melodyMemory',label:'Mémoire mélodique',ready:()=>((by.melody?.practiced||0)>=2),weak:()=>1-(by.melody?.mastery||0)},{kind:'harmonyFunction',label:'Harmonie fonctionnelle',ready:()=>((by.progression?.practiced||0)+(by.modulation?.practiced||0)>=2),weak:()=>1-((by.progression?.mastery||0)+(by.modulation?.mastery||0))/2},{kind:'polyrhythmTiming',label:'Polyrhythmie 3:2',ready:()=>((by.rhythm?.practiced||0)>=6&&(by.rhythm?.mastery||0)>=.5),weak:()=>1-(by.rhythm?.mastery||0)}],candidates=[];for(const d of defs){if(!d.ready())continue;const row=last[d.kind],stale=row?clamp((timestamp-row.time)/(14*DAY),0,1):1,priority=clamp(.68*stale+.32*clamp(d.weak(),0,1),0,1);candidates.push({...d,priority,lastTime:row?.time||0})}candidates.sort((a,b)=>b.priority-a.priority);const best=candidates[0];if(!best||best.priority<.55)return null;return{kind:best.kind,label:best.label,priority:best.priority,reason:best.lastTime?'mesure à renouveler':'mesure de calibration',requiresKeyboard:false}}
  ...
  return{SCHEMA,VERSION,DAY,HOUR,LEVELS,DEFAULT_SETTINGS,createState,sanitizeSettings,sanitizeState,migrateLegacy,ensureCard,betaStats,evidenceStats,contextStats,contextPriority,latentKeys,latentStats,challengeScalar,irtPrediction,retrievability,rawPrediction,correlatedRisk,prediction,updateCalibration,calibrationMetrics,modelComparison,tuningThresholdFromId,intonationProfile,intonationCalibrationMatrix,confusionGraph,moduleEnabled,transferSummary,competenceStage,placementReady,recordPlacement,prerequisiteReady,exposurePrerequisiteReady,isUnlocked,isExposureUnlocked,eligibleUnits,scoreUnit,selectUnit,markShown,contextKeys,updateAnswer,recordExposure,recordSession,recordPerformance,performancePolicy,performanceQualification,performanceEvidenceSignals,performanceTargetingByFamily,recommendedProbe,familySummary,summarize,planningSignals,recommendedPlan,exportState,stateBytes,seeded};
});
