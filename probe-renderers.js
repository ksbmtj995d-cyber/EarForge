...
(function(root,factory){const api=factory(typeof module==='object'&&module.exports?require('./audio-timeline.js'):root.EarForgeTimeline,typeof module==='object'&&module.exports?require('./music-events.js'):root.EarForgeMusicEvents);if(typeof module==='object'&&module.exports)module.exports=api;else root.EarForgeProbeRenderers=api;})(typeof globalThis!=='undefined'?globalThis:this,function(Timeline,Events){
 ...
 class IndependentRenderer{
  ...
  voice(midi,start,duration=.34,level=.55,dest=null,carrierVariant=0,pitchCurve=null){
   if(level===0)return start+duration;
   ...
   f.type='lowpass';f.frequency.value=Math.min(c.sampleRate*.43,6800,hz*(10+v*2));const floor=Math.max(Number.MIN_VALUE,level*.0001);g.gain.setValueAtTime(floor,start);g.gain.exponentialRampToValueAtTime(level,start+Math.min(.012,duration*.3));g.gain.exponentialRampToValueAtTime(floor,end+.07);f.connect(g).connect(dest||this.master);
   const peak=hz*Math.pow(2,Math.max(0,...(pitchCurve||[]).map(p=>p.semitones))/12);for(let i=0;i<3;i++){if(peak*ratios[i]>=c.sampleRate*.47)continue;const o=make('createOscillator');o.type=types[i];o.frequency.value=hz*ratios[i];if(pitchCurve)for(const [j,p]of pitchCurve.entries()){const frequency=hz*ratios[i]*Math.pow(2,p.semitones/12);j?o.frequency.exponentialRampToValueAtTime(frequency,start+p.t*duration):o.frequency.setValueAtTime(frequency,start);}o.connect(f);o.start(start);o.stop(end+.09);sources.push(o);}
   if(dest)nodes.push(dest);this.own(nodes,sources);return end+.09;
  }
  spatialVoice(midi,azimuth,start,duration=.38,level=.42,carrierVariant=0,pitchCurve=null){
   if(level===0)return start+duration;
   const c=this.context,p=this.track(c.createPanner()),rad=azimuth*Math.PI/180;p.panningModel='HRTF';p.distanceModel='inverse';p.refDistance=1;p.maxDistance=8;p.rolloffFactor=.25;const x=Math.sin(rad),z=-Math.cos(rad);if(p.positionX){p.positionX.setValueAtTime(x,start);p.positionY.setValueAtTime(0,start);p.positionZ.setValueAtTime(z,start);}else p.setPosition(x,0,z);p.connect(this.master);return this.voice(midi,start,duration,level,p,carrierVariant,pitchCurve);
  }
  click(frequency,start,level=.34){if(level===0)return start+.04;const c=this.context,o=this.track(c.createOscillator()),g=this.track(c.createGain());o.type='square';o.frequency.value=frequency;g.gain.setValueAtTime(.0001,start);g.gain.linearRampToValueAtTime(level,start+.002);g.gain.exponentialRampToValueAtTime(.0001,start+.035);o.connect(g).connect(this.master);o.start(start);o.stop(start+.04);this.own([o,g],[o]);return start+.04;}
  release(){for(const n of this.nodes){try{n.stop?.()}catch{}try{n.disconnect?.()}catch{}}this.nodes.clear();}
  async play(plan){
   plan=Events.plan(plan);for(const e of plan.events){if(e.releaseTime!==undefined||e.brightness!==undefined||e.colorVelocity!==undefined)throw new Error('Ce rendu indépendant ne réalise pas cette enveloppe expressive.');if(e.kind==='spectral'||e.spectral)throw new Error('Ce rendu indépendant ne restitue pas un spectre imposé.');if(e.kind==='drum'){if(e.instrument!=='click')throw new Error('Ce rendu indépendant ne restitue que des pulsations, pas un ensemble de batterie.');valid(e.frequency,40,8000,'Fréquence');}if(e.azimuth!=null)valid(e.azimuth,-180,180,'Position spatiale');if(e.carrierVariant!=null)valid(e.carrierVariant,0,1024,'Variante');}const band=c=>{if(c&&plan.events.some(e=>e.kind==='note'&&!Events.isSilent(e)&&mtof(e.pitch+(e.cents||0)/100+Math.max(0,...(e.pitchCurve||[]).map(p=>p.semitones)))>=c.sampleRate*.47))throw new Error('Hauteur au-delà de la bande de restitution');};band(this.context);this.stop();const epoch=this.epoch,c=await this.ensure();if(epoch!==this.epoch)throw new Error('Lecture annulée');band(c);
   const transport=new Timeline.Transport(c,{render:(e,at)=>{if(Events.isSilent(e))return at+e.duration;const pitch=e.pitch+(e.cents||0)/100,level=(e.level??(e.kind==='drum'?.34:.55))*(e.velocity??1);return e.kind==='drum'?this.click(e.frequency,at,level):e.azimuth==null?this.voice(pitch,at,e.duration,level,null,e.carrierVariant,e.pitchCurve):this.spatialVoice(pitch,e.azimuth,at,e.duration,level,e.carrierVariant,e.pitchCurve)},onEnd:r=>{if(r.status!=='ended')this.release();}});
 ...
 return Object.freeze({VERSION,IndependentRenderer,contract:Object.freeze({schema:'earforge.probe_renderer.v3',pitchCurves:true,microtonalPitch:true,imposedSpectrum:false,independentFromEarForgeAudio:true,profileReuse:false,sharedTransport:true,externalRecordingEquivalent:false,hrtf:true,visualRequired:false})});
});
