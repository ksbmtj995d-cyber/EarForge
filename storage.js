...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.EarForgeStorage=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function recordsFromState(state,{recent=false}={}){const out=[],push=(type,rows,limit)=>{const a=Array.isArray(rows)?rows:[],use=recent?a.slice(-limit):a;for(const row of use){const r=record(type,row);if(r)out.push(r);}};push('attempt',state?.attempts,RECENT.attempt);push('session',state?.sessions,RECENT.session);push('performance',state?.performance,RECENT.performance);push('placement',state?.placement?.history,RECENT.placement);push('singing',state?.singingAttempts,16);return out;}

 function checkIdentity(prior,row){
  if(prior&&stable(prior)!==stable(row)){const e=new Error('Identifiant d’historique répété avec des contenus différents.');e.code='EARFORGE_HISTORY_CONFLICT';throw e;}
 }
 function normalizeRecords(rows){const by=new Map();for(const raw of Array.isArray(rows)?rows:[]){const r=raw?.id&&raw?.type&&raw?.data?{id:String(raw.id),type:String(raw.type),time:cleanTime(raw.time),data:JSON.parse(JSON.stringify(raw.data))}:record(raw?.type||'event',raw?.data||raw);if(r){const prior=by.get(r.id);checkIdentity(prior,r);by.set(r.id,r);}}return[...by.values()].sort((a,b)=>a.time-b.time||a.id.localeCompare(b.id));}
 ...
 class HistoryQueue{
  constructor(){this.recentIds=new Map();this.syncQueue=Promise.resolve();this.retryRows=new Map();this.pendingOperations=0;this.lastError=null;this.recovery=null;}
  ...
  async persist(rows){if(this.recovery)throw new Error('Historique protégé : un remplacement doit être récupéré.');const next=new Map(this.retryRows);for(const row of rows){checkIdentity(next.get(row.id),row);next.set(row.id,row);}if(!next.size)return 0;try{const n=await this.writeRows([...next.values()]);this.retryRows.clear();this.lastError=null;return n;}catch(error){if(error.code!=='EARFORGE_HISTORY_CONFLICT')this.retryRows=next;throw error;}}
  ...
  async bootstrap(state){const all=recordsFromState(state),recent=recordsFromState(state,{recent:true});return this.enqueue(async()=>{const n=await this.persist(all);this.recentIds=new Map(recent.map(r=>[r.id,r]));return n;});}
  async syncRecent(state){const rows=recordsFromState(state,{recent:true});return this.enqueue(async()=>{const fresh=rows.filter(r=>{const previous=this.recentIds.get(r.id);checkIdentity(previous,r);return !previous;}),n=await this.persist(fresh);this.recentIds=new Map(rows.map(r=>[r.id,r]));return n;});}
  async count(){return this.enqueue(()=>this.countRows());}
  async exportAll(){return this.enqueue(async()=>{if(this.recovery)return normalizeRecords(this.recovery.previous);await this.persist([]);return this.readRows();});}
  ...
  async replaceDataset(rows,commit){
    if(!Array.isArray(rows)||typeof commit!=='function')throw new Error('Remplacement de données invalide.');
    const clean=normalizeRecords(rows);
    return this.enqueue(async()=>{
      if(this.recovery){await this.replaceRows(this.recovery.previous);this.recovery=null;}
      await this.persist([]);const previous=normalizeRecords(await this.readRows());
      await this.replaceRows(clean);
      try{const result=commit();if(result&&typeof result.then==='function')throw new Error('La validation du cache doit être synchrone.');}
      catch(error){
        try{await this.replaceRows(previous)}catch(rollback){
          this.recovery={previous,error:String(error.message||error),rollback:String(rollback.message||rollback)};
          const e=new Error('Remplacement incomplet : archive précédente conservée en mémoire. Exportez une sauvegarde avant de fermer cette page.');e.code='EARFORGE_REPLACEMENT_RECOVERY';throw e;
        }
        throw error;
      }
      this.recentIds.clear();this.retryRows.clear();this.lastError=null;return clean.length;
    });
  }
 ...
 class MemoryHistoryStore extends HistoryQueue{
  ...
  async writeRows(rows){for(const r of rows)checkIdentity(this.map.get(r.id),r);for(const r of rows)if(!this.map.has(r.id))this.map.set(r.id,r);return rows.length;}
 ...
 class IndexedHistoryStore extends HistoryQueue{
  ...
  async transaction(mode,fn){const db=await this.open();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,mode),s=tx.objectStore(STORE);let result;tx.oncomplete=()=>resolve(typeof result==='function'?result():result);tx.onerror=()=>reject(tx.error||new Error('Archive locale indisponible'));tx.onabort=()=>reject(tx.error||new Error('Archive locale annulée'));const fail=e=>{try{tx.abort();}catch{}reject(e);};try{result=fn(s,tx,fail);}catch(e){fail(e);}});}
  async prepared(rows){const encoded=[];for(const r of rows)encoded.push(await encodeRecord(r));return encoded;}
  async writeRows(rows){
   if(!rows.length)return 0;
   const encoded=await this.prepared(rows);

   const before=await this.transaction('readonly',s=>{const requests=rows.map(r=>s.get(r.id));return()=>requests.map(r=>r.result);});
   for(let i=0;i<rows.length;i++)if(before[i]!==undefined)checkIdentity(await decodeRecord(before[i]),rows[i]);
   const snapshots=before.map(stable);

   await this.transaction('readwrite',(s,tx,fail)=>{
    for(let i=0;i<rows.length;i++){
     const request=s.get(rows[i].id);
     request.onsuccess=()=>{try{
      if(stable(request.result)!==snapshots[i]){const e=new Error('Historique modifié dans une autre page ; réessayez la synchronisation.');e.code='EARFORGE_HISTORY_CONCURRENT';throw e;}
      if(request.result===undefined)s.add(encoded[i]);
     }catch(e){fail(e);}};
    }
   });
   return rows.length;
 ...
 return{DB_NAME,STORE,RECENT,record,recordsFromState,normalizeRecords,MAX_ROW_BYTES,MemoryHistoryStore,IndexedHistoryStore,NullHistoryStore,createHistoryStore};
});
